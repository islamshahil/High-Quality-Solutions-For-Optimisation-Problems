...
class environment():
    ...
    def geocoding_api(self, address):  # 2 output: status, position
        ...
        geocode_url = geocode_api + urllib.parse.urlencode({'address': address}) + "$key=AIzaSyDms9WSV9gyhWlRNOF_eQePiUoNp-EcPFU"
    ...
    def elevation_api(self, location):  # 2 output: elevation, resolution
        ...
        elevation_url = elevation_api + urllib.parse.urlencode({'locations': location}) + "$key=AIzaSyDms9WSV9gyhWlRNOF_eQePiUoNp-EcPFU"
    ...
    def directions_api(self, origin, destination):  # 5 output: status, steps, bound
        ...
        directions_url = directions_api + urllib.parse.urlencode({'origin': origin}) + '&' + urllib.parse.urlencode(
            {'destination': destination}) + '&' + urllib.parse.urlencode({'units': 'metric'}) + "$key=AIzaSyDms9WSV9gyhWlRNOF_eQePiUoNp-EcPFU"
    ...
    def make_map(self):
        ...
        if destination_position_num == ('g','g'):
            destination_position_num = (40.445283,-86.948429)
